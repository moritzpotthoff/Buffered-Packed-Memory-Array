...
private:
    ...
    using PmaEntryType = PmaEntry<KeyType, ValueType, NULL_KEY, KeyOf, KeyCompare, Allocator, MinSize, MaxSize, SDS_TYPE>;
...
public:
    PmaEntry() : block(nullptr),
        head(NULL_KEY),
        size(0),
        comp() {}

    PmaEntry(BlockPointer b, const KeyType &v, BlockSizeType s) : block(b),
        head(v),
        size(s) {}
...
public:
    ...
    friend std::ostream& operator<<(std::ostream& out, const PmaEntry& entry) {
