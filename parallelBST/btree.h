...
#define BTREE_FRIENDS \
    ...
    friend class ::btree_print; \
    \
    template <typename TBtree> \
    friend struct ::TParallelSplit;
#endif

//#define BTREE_COUNT_NODES

#ifdef BTREE_COUNT_NODES

#include <atomic>

static std::atomic<uint32_t> VisitedNodes(0);

#define BTREE_VISIT_NODE_RESET stx::VisitedNodes = 0;
#define BTREE_VISIT_NODE ++stx::VisitedNodes;
#define BTREE_VISITED_NODES stx::VisitedNodes

#else

#define BTREE_VISIT_NODE_RESET
#define BTREE_VISIT_NODE
#define BTREE_VISITED_NODES 0
 ...
 */
    ...
    public:
        ...
        //static const unsigned short minleafslots = (leafslotmax / 4);
        static const unsigned short minleafslots = (leafslotmax / 2);

        /// Computed B+ tree parameter: The minimum number of key slots used
        /// in an inner node. If fewer slots are used, the inner node will be
        /// merged or slots shifted from it's siblings.
        //static const unsigned short mininnerslots = (innerslotmax / 16);
    ...
    private:
        ...
        struct inner_node : public node
        {
            ...
            typedef typename std::allocator_traits<_Alloc>::rebind_alloc<inner_node> alloc_type;
            //typedef typename _Alloc::rebind<inner_node>::other alloc_type;
            ...
            inline void copy_slots(const inner_node& n)
            {
            ...
            inline void swap(inner_node& n)
            {
                inner_node tmp_node;

                tmp_node.level = node::level;
                tmp_node.copy_slots(*this);

                node::level = n.level;
                copy_slots(n);

                n.level = tmp_node.level;
                n.copy_slots(tmp_node);
            }
        };
        ...
        struct leaf_node : public node
        {
            ...
            typedef typename std::allocator_traits<_Alloc>::rebind_alloc<leaf_node> alloc_type;
            //typedef typename _Alloc::rebind<leaf_node>::other alloc_type;
            ...
            inline void copy_slots(const leaf_node& leaf)
            {
                std::copy(leaf.slotkey, leaf.slotkey + leaf.slotuse, slotkey);

                if (!used_as_set)
                    std::copy(leaf.slotdata, leaf.slotdata + leaf.slotuse, slotdata);
                node::slotuse = leaf.slotuse;
            }

            inline void swap(leaf_node& leaf)
            {
                leaf_node tmp_node;
                ...
                leaf.level = tmp_node.level;
                leaf.copy_slots(tmp_node);
    ...
    private:
        ...
        inline void free_node(node *n)
        {
            if (n->isleafnode()) {
                ...
                std::allocator_traits<typename leaf_node::alloc_type>::deallocate(a, ln, 1);
                //a.destroy(ln);
                //a.deallocate(ln, 1);
                //m_stats.leaves--;
            }
                ...
                std::allocator_traits<typename inner_node::alloc_type>::deallocate(a, in, 1);
                //a.destroy(in);
                //a.deallocate(in, 1);
                //m_stats.innernodes--;
            }
    ...
    public:
        ...
        {
            return insert_start(key, data_type());
        }
        ...
        template <typename InputIterator>
        inline void insert_sorted(InputIterator first, InputIterator last)
        {
            InputIterator next = first;

            InputIterator prevNext = first;
            while (next != last)
            {
                next = insert_sorted_start(next, last);
                //PRINT("Diff", next - prevNext)
                prevNext = next;
            }
        }

        inline bool join_less(btree_self& other_tree)
        {
            if (empty())
            {
                swap(other_tree);
            ...
            if (m_root->level < other_tree.m_root->level)
                return false;

//            key_type this_first = m_headleaf->slotkey[0];
            key_type other_last = other_tree.m_tailleaf->slotkey[other_tree.m_tailleaf->slotuse - 1];
            key_type key = other_last;

//            if (!allow_duplicates && key_equal(this_first, other_last)
//                || !key_lessequal(other_last, this_first))
//                return false;

            bool res = join_less_start(key, other_tree);

            // clear other_tree if join was successful
            if (res)
            {
                other_tree.m_root = NULL;
                other_tree.m_headleaf = other_tree.m_tailleaf = NULL;
                other_tree.m_stats = tree_stats();
            }

            return res;
        }

        inline bool join_greater(btree_self& other_tree)
        {
            if (empty())
            {
                swap(other_tree);
            ...
            key_type this_last = m_tailleaf->slotkey[m_tailleaf->slotuse - 1];
//            key_type other_first = other_tree.m_headleaf->slotkey[0];
            key_type key = this_last;

//            if (!allow_duplicates && key_equal(this_last, other_first)
//                || !key_lessequal(this_last, other_first))
//                return false;

            bool res = join_greater_start(key, other_tree);
        ...
        inline bool join_less(key_type key, btree_self& other_tree)
        {
            if (empty())
            {
                swap(other_tree);
        ...
        inline bool join_greater(key_type key, btree_self& other_tree)
        {
            if (empty())
            {
                swap(other_tree);
        ...
        inline uint32_t height_danger() const
        {
            return m_root->level + 1;
        }
        ...
        {
            return size_slow_rec(m_root);
        }

        void to_vector(std::vector<key_type>& data) const
        {
            if (empty())
                return;
            to_vector_rec(m_root, data);
        }

        typedef std::vector<node*> spine;

        void add_root(spine& spine)
        {
            spine.push_back(m_root);
        }

        void build_spines(spine& left_spine, spine& right_spine)
        {
            left_spine.reserve(2 * (m_root->level + 1));
            right_spine.reserve(2 * (m_root->level + 1));
            left_spine.resize(m_root->level + 1);
            right_spine.resize(m_root->level + 1);

            node* n = m_root;
            size_t level = m_root->level;
            while (!n->isleafnode())
            {
                left_spine[level] = n;
                --level;
                n = static_cast<inner_node*>(n)->childid[0];
            }
            left_spine[level] = n;
            ...
            while (!n->isleafnode())
            {
                right_spine[level] = n;
                --level;
                n = static_cast<inner_node*>(n)->childid[n->slotuse];
            }
            right_spine[level] = n;
        }

        inline bool join_less(key_type key, spine& left_spine, btree_self& other_tree, spine& right_spine, key_type* new_key)
        {
            size_t level = other_tree.m_root->level;
            node* n = left_spine[level];

//            assert(n->level == other_tree.m_root->level);
//
//            if (empty() || other_tree.empty() || m_root->level < other_tree.m_root->level)
//                return false;
            ...
            if (n == m_root)
            {
                ...
                key_type newkey;
                node* newchild = NULL;
                node* other_node = other_tree.m_root;
                EJoinType type;

                if (n->isleafnode())
                    type = join_leaves(other_node, n, key, &newkey, &newchild);
                else
                    type = join_inner(other_node, n, key, &newkey, &newchild);

                if (type == MERGED)
                {
                    if (!n->isleafnode())
                        static_cast<inner_node*>(n)->copy_slots(*static_cast<inner_node*>(other_node));
                    else
                        static_cast<leaf_node*>(n)->copy_slots(*static_cast<leaf_node*>(other_node));
                    free_node(other_tree.m_root);
                }
                ...
                if (type == NO_DIFF || type == SPLITED)
                {
                    ...
                    newroot->childid[0] = other_node;
                    newroot->childid[1] = m_root;

                    newroot->slotuse = 1;

                    m_root = newroot;

                    left_spine.back() = other_node;
                    right_spine.push_back(m_root);
                    left_spine.push_back(m_root);
                }
                res = true;
            }
            else
                res = join_left_spine(left_spine, key, n, other_tree, new_key);
        ...
        bool check_spines(spine& left_spine, spine& right_spine)
        {
            for (size_t i = 0; i < left_spine.size(); ++i)
                if (i != left_spine[i]->level)
                {
                    std::cout << "Left spine incorrect. Correct level: " << i
                              << ", Current level: " << right_spine[i]->level << std::endl;
                    return false;
                }

            for (size_t i = 0; i < right_spine.size(); ++i)
                if (i != right_spine[i]->level)
                {
                    std::cout << "Right spine incorrect. Correct level: " << i
                              << ", Current level: " << right_spine[i]->level << std::endl;
                    return false;
                }
        ...
        inline bool join_greater(key_type key, spine& right_spine, btree_self& other_tree, spine& left_spine, key_type* new_key)
        {
            size_t level = other_tree.m_root->level;
            node* n = right_spine[level];
...
//                return false;
            ...
            if (n == m_root)
            {
                ...
                if (n->isleafnode())
                    join_leaves(n, other_node, key, &newkey, &newchild);
                else
                    join_inner(n, other_node, key, &newkey, &newchild);
                ...
                res = join_right_spine(right_spine, key, n, other_tree, new_key);
        ...
        key_type min_key() const
        {
            ...
            return m_headleaf->slotkey[0];
        }

        key_type min_key_slow() const
        {
            if (empty())
                return std::numeric_limits<key_type>::min();
            ...
            while (!n->isleafnode())
                n = static_cast<const inner_node*>(n)->childid[0];

            return static_cast<const leaf_node*>(n)->slotkey[0];
        }
        ...
        key_type max_key() const
        {
            ...
            return m_tailleaf->slotkey[m_tailleaf->slotuse - 1];
        }

        key_type max_key_slow() const
        {
            if (empty())
                return std::numeric_limits<key_type>::min();
            ...
            while (!n->isleafnode())
                n = static_cast<const inner_node*>(n)->childid[n->slotuse];

            return static_cast<const leaf_node*>(n)->slotkey[n->slotuse - 1];
        }

        void split(btree_self& left, key_type key, btree_self& right)
        {
            if (empty())
                return;
            split_recursive(m_root, left, key, right);
            if (m_root)
            {
                m_root = NULL;
                m_headleaf = m_tailleaf = NULL;
    ...
    private:
        using TPairTreeKey = std::pair<btree_self, key_type>;

        void split_recursive(node* n, btree_self& left, key_type key, btree_self& right)
        {
            if (!n->isleafnode())
            {
                ...
                btree_self bottom_left, bottom_right;
                split_recursive(child, bottom_left, key, bottom_right);

                new_left.first.join_greater(new_left.second, bottom_left);
                new_right.first.join_less(new_right.second, bottom_right);
        ...
        void split_inner_node(inner_node* n, int slot, TPairTreeKey& left, TPairTreeKey& right)
        {
            size_t level = n->level;
            inner_node* new_left_root = NULL;
            inner_node* new_right_root = NULL;
            ...
            int slotuse = n->slotuse;
            if (slot > 0)
            {
                left.second = n->slotkey[slot - 1];

                // if node of the spliting tree
                // will be in the left tree then
                // no need to copy only change
                // its slotuse
                if (n != new_left_root)
                {
                    std::copy(n->slotkey, n->slotkey + slot - 1, new_left_root->slotkey);
            ...
            if (slot < slotuse)
            {
                ...
                std::copy(n->slotkey + slot + 1, n->slotkey + slotuse, new_right_root->slotkey);
                std::copy(n->childid + slot + 1, n->childid + slotuse + 1, new_right_root->childid);
                new_right_root->slotuse = slotuse - (slot + 1);
            }

            left.first.m_root = new_left_root;
            right.first.m_root = new_right_root;
            if (new_left_root && new_left_root->slotuse == 0)
            {
                left.first.m_root = new_left_root->childid[0];
                free_node(new_left_root);
            }

            if (new_right_root && new_right_root->slotuse == 0)
            {
                right.first.m_root = new_right_root->childid[0];
                free_node(new_right_root);
            }
        }

        void split_leaf_node(leaf_node* n, int slot, btree_self& left, btree_self& right)
        {
            leaf_node* new_left_root = NULL;
            leaf_node* new_right_root = NULL;
            ...
            if (slot > 0)
            {
                if (n != new_left_root)
                {
                    std::copy(n->slotkey, n->slotkey + slot, new_left_root->slotkey);
                    if (!used_as_set)
            ...
            if (slot < slotuse)
            {
                std::copy(n->slotkey + slot, n->slotkey + slotuse, new_right_root->slotkey);
                if (!used_as_set)
                    std::copy(n->slotdata + slot, n->slotdata + slotuse, new_right_root->slotdata);
                new_right_root->slotuse = slotuse - slot;
            }
            left.m_root = new_left_root;
            right.m_root = new_right_root;
        }

        // *** Private copy to vector

        void to_vector_rec(node* n, std::vector<key_type>& data) const
        {
            if (!n->isleafnode())
            {
                inner_node* inner = static_cast<inner_node*>(n);
                for (size_t slot = 0; slot <= inner->slotuse; ++slot)
                    to_vector_rec(inner->childid[slot], data);
            }
            else
            {
                leaf_node* leaf = static_cast<leaf_node*>(n);

                for (size_t slot = 0; slot < leaf->slotuse; ++slot)
                {
                    data.push_back(leaf->slotkey[slot]);
                }
            }
        }

        size_t size_slow_rec(node* n) const
        {
            size_t size = 0;
            ...
            if (!n->isleafnode())
            {
                inner_node* inner = static_cast<inner_node*>(n);
                for (size_t slot = 0; slot <= inner->slotuse; ++slot)
                    size += size_slow_rec(inner->childid[slot]);
            }
            else
            {
                leaf_node* leaf = static_cast<leaf_node*>(n);
        ...
        // *** Private Join with spine functions
        inline bool join_left_spine(spine& spine, key_type key, node* n, btree_self& other_tree, key_type* new_key)
        {
            BTREE_VISIT_NODE
            node* other_node = other_tree.m_root;

            key_type new_sep_key;
            node* newchild = NULL;
            EJoinType type;

            if (!n->isleafnode())
                type = join_inner(other_node, n, key, &new_sep_key, &newchild);
            else
                type = join_leaves(other_node, n, key, &new_sep_key, &newchild);
            ...
            if (type == SPLITED || type == NO_DIFF)
            {
                // visit parent
                BTREE_VISIT_NODE
                inner_node* parent = static_cast<inner_node*>(spine[n->level + 1]);
                if (parent->slotuse < innerslotmax)
                {
                    std::copy_backward(parent->slotkey, parent->slotkey + parent->slotuse,
                                       parent->slotkey + parent->slotuse + 1);

                    std::copy_backward(parent->childid, parent->childid + parent->slotuse + 1,
                                       parent->childid + parent->slotuse + 2);

                    parent->slotkey[0] = new_sep_key;
                    parent->childid[0] = other_node;
                    spine[n->level] = other_node;
                    ++parent->slotuse;
                    return true;
                }
                else
                {
                    // delete link to node from parent
                    *new_key = parent->slotkey[0];

                    std::copy(parent->slotkey + 1,
                              parent->slotkey + parent->slotuse,
                              parent->slotkey);

                    std::copy(parent->childid + 1,
                              parent->childid + parent->slotuse + 1,
                              parent->childid);
                    --parent->slotuse;

                    // join subtrees with roots n and other_node
                    inner_node *newroot = allocate_inner(n->level + 1);
                    newroot->slotkey[0] = new_sep_key;
        ...
        inline bool join_right_spine(spine& spine, key_type key, node* n, btree_self& other_tree, key_type* new_key)
        {
            BTREE_VISIT_NODE
            node* other_node = other_tree.m_root;
            ...
            if (!n->isleafnode())
                type = join_inner(n, other_node, key, &new_sep_key, &newchild);
            else
                type = join_leaves(n, other_node, key, &new_sep_key, &newchild);
            ...
            if (type == SPLITED || type == NO_DIFF)
            {
                ...
                if (parent->slotuse < innerslotmax)
                {
                    parent->slotkey[parent->slotuse] = new_sep_key;
                    parent->childid[parent->slotuse + 1] = other_node;
                    spine[n->level] = other_node;
                    ++parent->slotuse;
                ...
                else
                {
                    // delete link to node from parent
                    --parent->slotuse;
                    *new_key = parent->slotkey[parent->slotuse];
        ...
        EJoinType join_leaves(node* n, node* other_node, const key_type& key, key_type* newkey, node** newchild)
        {
            BTREE_VISIT_NODE
            *newkey = key;

            leaf_node* leaf = static_cast<leaf_node*>(n);
            leaf_node* other_leaf = static_cast<leaf_node*>(other_node);

            if (leafslotmax >= leaf->slotuse + other_leaf->slotuse)
            {
                // elements of both leaves can be placed
                // in one node
                int slot = leaf->slotuse;

                std::copy(other_leaf->slotkey, other_leaf->slotkey + other_leaf->slotuse,
                          leaf->slotkey + slot);
                if (!used_as_set)
                    std::copy(other_leaf->slotdata, other_leaf->slotdata + other_leaf->slotuse,
                              leaf->slotdata + slot);
            ...
            {
                // need redistribute nodes between roots
                if (leaf->slotuse < minleafslots)
                {
                    // copy from other root
                    // to this root
                    std::copy(other_leaf->slotkey,
                              other_leaf->slotkey + (minleafslots - leaf->slotuse),
                              leaf->slotkey + leaf->slotuse);

                    if (!used_as_set)
                        std::copy(other_leaf->slotdata,
                                  other_leaf->slotdata + (minleafslots - leaf->slotuse),
                                  leaf->slotdata + leaf->slotuse);

                    *newkey = leaf->slotkey[leaf->slotuse + minleafslots - leaf->slotuse - 1];

                    std::copy(other_leaf->slotkey + minleafslots - leaf->slotuse,
                              other_leaf->slotkey + other_leaf->slotuse,
                              other_leaf->slotkey);
                    if (!used_as_set)
                        std::copy(other_leaf->slotdata + minleafslots - leaf->slotuse,
                                  other_leaf->slotdata + other_leaf->slotuse,
                                  other_leaf->slotdata);

                    other_leaf->slotuse -= minleafslots - leaf->slotuse;
                    leaf->slotuse = minleafslots;
                    *newchild = other_leaf;
                    return SPLITED;
                }
                else if (other_leaf->slotuse < minleafslots)
                {
                    // copy from this root
                    // to other root
                    std::copy_backward(other_leaf->slotkey,
                                       other_leaf->slotkey + other_leaf->slotuse,
                                       other_leaf->slotkey + minleafslots);

                    if (!used_as_set)
                        std::copy_backward(other_leaf->slotdata,
                                           other_leaf->slotdata + other_leaf->slotuse,
                                           other_leaf->slotdata + minleafslots);

                    std::copy_backward(leaf->slotkey + leaf->slotuse - (minleafslots - other_leaf->slotuse),
                                       leaf->slotkey + leaf->slotuse,
                                       other_leaf->slotkey + (minleafslots - other_leaf->slotuse));

                    if (!used_as_set)
                        std::copy_backward(leaf->slotdata + leaf->slotuse - (minleafslots - other_leaf->slotuse),
                                           leaf->slotdata + leaf->slotuse,
                                           other_leaf->slotdata + (minleafslots - other_leaf->slotuse));

                    *newkey = leaf->slotkey[leaf->slotuse - (minleafslots - other_leaf->slotuse) - 1];

                    leaf->slotuse -= minleafslots - other_leaf->slotuse;
                    other_leaf->slotuse = minleafslots;
                    *newchild = other_leaf;
                    return SPLITED;
                }
        ...
        EJoinType join_inner(node* n, node* other_node, const key_type& key, key_type* newkey, node** newchild)
        {
            *newkey = key;

            inner_node* inner = static_cast<inner_node*>(n);
            inner_node* other_inner = static_cast<inner_node*>(other_node);

            if (innerslotmax >= inner->slotuse + other_inner->slotuse + 1)
            {
                // elements of both roots can be placed
                // in one node
                int slot = inner->slotuse;
                inner->slotkey[slot] = key;

                std::copy(other_inner->slotkey, other_inner->slotkey + other_inner->slotuse,
                          inner->slotkey + slot + 1);

                std::copy(other_inner->childid, other_inner->childid + other_inner->slotuse + 1,
            ...
            {
                // need redistribure nodes between roots
                if (inner->slotuse < mininnerslots)
                {
                    // copy from other root
                    // to this root
                    ...
                    std::copy(other_inner->slotkey,
                              other_inner->slotkey + (mininnerslots - inner->slotuse),
                              inner->slotkey + inner->slotuse + 1);

                    std::copy(other_inner->childid,
                              other_inner->childid + (mininnerslots - inner->slotuse),
                              inner->childid + inner->slotuse + 1);

                    *newkey = inner->slotkey[inner->slotuse + 1 + mininnerslots - inner->slotuse - 1];

                    std::copy(other_inner->slotkey + (mininnerslots - inner->slotuse),
                              other_inner->slotkey + other_inner->slotuse,
                              other_inner->slotkey);

                    std::copy(other_inner->childid + (mininnerslots - inner->slotuse),
                              other_inner->childid + other_inner->slotuse + 1,
                              other_inner->childid);

                    other_inner->slotuse -= mininnerslots - inner->slotuse;
                    inner->slotuse = mininnerslots;
                    *newchild = other_inner;
                    return SPLITED;
                }
                else if (other_inner->slotuse < mininnerslots)
                {
                    // copy from this root
                    // to other root
                    std::copy_backward(other_inner->slotkey,
                                       other_inner->slotkey + other_inner->slotuse,
                                       other_inner->slotkey + mininnerslots);

                    std::copy_backward(other_inner->childid,
                                       other_inner->childid + other_inner->slotuse + 1,
                                       other_inner->childid + mininnerslots + 1);

                    other_inner->slotkey[mininnerslots - other_inner->slotuse - 1] = key;

                    std::copy_backward(inner->slotkey + inner->slotuse
                                       - (mininnerslots - other_inner->slotuse - 1),
                                       inner->slotkey + inner->slotuse,
                                       other_inner->slotkey + (mininnerslots - other_inner->slotuse - 1));

                    std::copy_backward(inner->childid + inner->slotuse + 1
                                       - (mininnerslots - other_inner->slotuse),
                                       inner->childid + inner->slotuse + 1,
                                       other_inner->childid + (mininnerslots - other_inner->slotuse));

                    *newkey = inner->slotkey[inner->slotuse
                                             - (mininnerslots - other_inner->slotuse)];

                    inner->slotuse -= mininnerslots - other_inner->slotuse - 1 + 1;
                    other_inner->slotuse = mininnerslots;
                    *newchild = other_inner;
                    return SPLITED;
                }
        ...
        bool join_less_start(const key_type& key, btree_self& other_tree)
        {
            BTREE_VISIT_NODE
            bool r;
            node *newchild = NULL;
            ...
            if (m_root->level > other_tree.m_root->level)
                r = join_less_descend(m_root, key, other_tree, &newkey, &newchild, &leaves_merged);
            else
            {
                r = true;
                if (m_root->isleafnode())
                {
                    leaf_node* root = static_cast<leaf_node*>(m_root);
                    leaf_node* other_root = static_cast<leaf_node*>(other_tree.m_root);

                    leaves_merged = join_leaves(other_root, root, key, &newkey, &newchild);

                    if (leaves_merged == MERGED)
                    {
                        root->copy_slots(*other_root);
                        free_node(other_tree.m_root);
                    }
                    ...
                    if (leaves_merged == NO_DIFF || leaves_merged == SPLITED)
                    {
                        ...
                        newchild = other_tree.m_root;
                    }
                }
                else
                {
                    inner_node* root = static_cast<inner_node*>(m_root);
                    inner_node* other_root = static_cast<inner_node*>(other_tree.m_root);

                    EJoinType nodes_merged = join_inner(other_root, root, key, &newkey, &newchild);
            ...
            if (r)
            {
                m_stats.itemcount += other_tree.m_stats.itemcount;
                m_stats.innernodes += other_tree.m_stats.innernodes;
                m_stats.leaves += other_tree.m_stats.leaves;
            }
        ...
        bool join_greater_start(const key_type& key, btree_self& other_tree)
        {
            BTREE_VISIT_NODE
            bool r;
            node* newchild = NULL;
            ...
            if (m_root->level > other_tree.m_root->level)
                r = join_greater_descend(m_root, key, other_tree, &newkey, &newchild, &leaves_merged);
            else
            {
                r = true;
                ...
                if (m_root->isleafnode())
                {
                    leaves_merged = join_leaves(m_root, other_tree.m_root, key, &newkey, &newchild);
                }
                else
                {
                    nodes_merged = join_inner(m_root, other_tree.m_root, key, &newkey, &newchild);
                }
        ...
        bool join_less_descend(node* n, const key_type& key, btree_self& other_tree, key_type* splitkey, node** splitnode, EJoinType* leaves_merged)
        {
            BTREE_VISIT_NODE
            if (!n->isleafnode())
            {
                ...
                BTREE_PRINT("btree::join_descend into " << inner->childid[inner->slotuse - 1]);

                bool r;
                key_type newkey;
                node *newchild = NULL;

                if (n->level > other_tree.m_root->level)
                    // level of node which are on
                    // the right spine of this tree(*this)
                    // greater than other_tree's level plus one then
                    // go down by the right spine
                    r = join_less_descend(inner->childid[0], key, other_tree, &newkey, &newchild, leaves_merged);
                else
                {
                    // levels of trees equal.
                    // check if duplicates are allow
                    // and insert new tree as if it was
                    // propagated from below
                    inner_node* inner = static_cast<inner_node*>(n);
                    inner_node* other_inner = static_cast<inner_node*>(other_tree.m_root);

                    EJoinType type = join_inner(other_inner, inner, key, splitkey, splitnode);

                    if (type == MERGED)
                    {
                        inner->copy_slots(*other_inner);
                        free_node(other_tree.m_root);
                    }
                ...
                if (newchild)
                {
                    ...
                    if (inner->isfull())
                    {
                        ...
                        BTREE_PRINT("btree::insert_descend done split_inner: putslot: " << 0 << " putkey: " << newkey << " upkey: " << *splitkey);
                    ...
                    std::copy_backward(inner->slotkey, inner->slotkey + inner->slotuse,
                                       inner->slotkey + inner->slotuse + 1);
                    std::copy_backward(inner->childid, inner->childid + inner->slotuse + 1,
            ...
            {
                leaf_node* leaf =  static_cast<leaf_node*>(n);
                leaf_node* other_leaf = static_cast<leaf_node*>(other_tree.m_root);

                EJoinType type = join_leaves(other_leaf, leaf, key, splitkey, splitnode);

                if (type == MERGED)
                {
                    leaf->copy_slots(*other_leaf);
                    free_node(other_tree.m_root);
                }

                if (type == NO_DIFF || type == SPLITED)
                {
                    leaf->swap(*other_leaf);
                    *splitnode = other_tree.m_root;
                }
        ...
        bool join_greater_descend(node* n, const key_type& key, btree_self& other_tree, key_type* splitkey, node** splitnode, EJoinType* leaves_merged)
        {
            BTREE_VISIT_NODE
            if (!n->isleafnode())
            {
                ...
                if (n->level > other_tree.m_root->level)
                    ...
                    // greater than other_tree's level plus one then
                    // go down by the right spine
                    r = join_greater_descend(inner->childid[inner->slotuse], key, other_tree, &newkey, &newchild, leaves_merged);
                else
                {
                    // levels of trees equal.
                    // check if duplicates are allow
                    // and insert new tree as if it was
                    // propagated from below
                    EJoinType type = join_inner(n, other_tree.m_root, key, splitkey, splitnode);
                    if (type == MERGED)
                        free_node(other_tree.m_root);
            ...
            else
            {
                ...
                EJoinType type = join_leaves(leaf, other_leaf, key, splitkey, splitnode);
                if (type == MERGED)
                {
                    free_node(other_tree.m_root);
                    *leaves_merged = type;
        ...
        template <typename InputIterator>
        inline InputIterator insert_sorted_start(InputIterator first, InputIterator last)
        {
            node *newchild = NULL;
            key_type newkey = key_type();
            ...
            key_type upper_key = std::numeric_limits<key_type>::max();

            InputIterator r = insert_sorted_descend(m_root, first, last, upper_key, &newkey, &newchild);
        ...
        */
        ...
        template <typename InputIterator>
        InputIterator insert_sorted_descend(node* n, InputIterator first, InputIterator last,
                                                             key_type& upper_key, key_type* splitkey, node** splitnode)
        {
            key_type key = *first;
            ...
            if (!n->isleafnode())
            {
                ...
                while (key < upper_key || !(upper_key < key))
                {
                    key_type newkey = key_type();
                    node *newchild = NULL;
                    ...
                    key_type new_upper_key = upper_key;
                    if (slot < inner->slotuse)
                        new_upper_key = inner->slotkey[slot];

                    if (!allow_duplicates && !(key < new_upper_key))
                        return ++first;
                    else
                    {
                        if (inner->childid[slot]->isleafnode() && slot + 1 < inner->slotuse)
                        {
                            leaf_node* next_leaf = static_cast<leaf_node*>(inner->childid[slot + 1]);
                            new_upper_key = next_leaf->slotkey[0];
                        }
                        first = insert_sorted_descend(inner->childid[slot], first, last, new_upper_key, &newkey,
                                                      &newchild);

                        if (inner->childid[slot]->isleafnode() && slot < inner->slotuse && !newchild)
                        {
                            inner->slotkey[slot] = new_upper_key;
                        }
                    }
            ...
            else // n->isleafnode() == true
            {
                ...
                if (!allow_duplicates && slot < leaf->slotuse && key_equal(key, leaf->slotkey[slot])) {
                    ...
                    if (first == last)
                    {
                        //PRINT("Diff", first - oldFirst);
                        upper_key = leaf->slotkey[leaf->slotuse - 1];
                        return first;
                    }
                }
                ...
                // move items and put data item into correct data slot
                //leaf_node* split_leaf = static_cast<leaf_node*>(*splitnode);

                if (*splitnode)
                    //first = merge_leaf(leaf, split_leaf->slotkey[0] - 1, first, last);
                    first = merge_leaf(leaf, *splitkey, first, last);
                else
                    first = merge_leaf(leaf, upper_key, first, last);
                ...
                if (*splitnode)
                {
                    ...
                    // last slot of the old node. then the splitkey must be
                    // updated.
                    *splitkey = leaf->slotkey[leaf->slotuse - 1];

                    //first = merge_leaf(split_leaf, upper_key, first, last);
                    return first;
                }
        ...
        template <typename InputIterator>
        InputIterator merge_leaf(leaf_node* leaf, const key_type& upper_key,
                                                  InputIterator first, InputIterator last)
        {
            InputIterator last_for_leaf = std::upper_bound(first, last, upper_key);
            size_t new_keys = std::min<size_t>(leafslotmax - leaf->slotuse, last_for_leaf - first);
            ...
            if (new_keys > 1)
            {
                ...
                std::merge(leaf->slotkey, leaf->slotkey + leaf->slotuse,
                           first, first + new_keys, new_leaf.slotkey, key_compare());

                new_leaf.slotuse = leaf->slotuse + new_keys;
//                size_t new_slotuse = std::unique(new_leaf.slotkey, new_leaf.slotkey + new_leaf.slotuse)
//                                     - new_leaf.slotkey;
//
//                new_leaf.slotuse = new_slotuse;
                leaf->swap(new_leaf);
            }
            else
            {
                ...
                while (!allow_duplicates && slot < leaf->slotuse && key_equal(key, leaf->slotkey[slot])) {
                    while (*first == key && first != last)
                        ++first;
...
private:
    ...
    public:
        ...
        void verify_node_no_leaf_links() const
        {
            key_type minkey, maxkey;
            tree_stats vstats;

            if (m_root)
                verify_node_no_leaf_links(m_root, &minkey, &maxkey, vstats);
        }
    ...
    private:
        ...
        void verify_node_no_leaf_links(const node* n, key_type* minkey, key_type* maxkey, tree_stats &vstats) const
        {
            BTREE_PRINT("verifynode " << n);
            ...
            else // !n->isleafnode()
            {
                ...
                for(unsigned short slot = 0; slot <= inner->slotuse; ++slot)
                {
                    ...
                    assert(subnode->level + 1 == inner->level);
                    verify_node_no_leaf_links(subnode, &subminkey, &submaxkey, vstats);
    ...
    public:
        void fix_leaf_links()
        {
            m_tailleaf = fix_leaf_links_rec(m_root, nullptr);
        }

        //Fixes the leaf links in the subtree. Returns a pointer to the
        //last leaf in the subtree.
        leaf_node *fix_leaf_links_rec(node* node, leaf_node *last_previous_leaf)
        {
            if (node->isleafnode())
            {
                leaf_node *leafnode = static_cast<leaf_node*>(node);

                if (last_previous_leaf != nullptr) {
                    last_previous_leaf->nextleaf = leafnode;
                }
            ...
            else
            {
                ...
                leaf_node *last_leaf = last_previous_leaf;

                for (unsigned short slot = 0; slot < innernode->slotuse + 1; ++slot)
                {
                    last_leaf = fix_leaf_links_rec(innernode->childid[slot], last_leaf);
                }
                return last_leaf;
            }
        }
